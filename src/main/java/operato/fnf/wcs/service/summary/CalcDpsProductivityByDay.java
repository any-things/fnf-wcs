...
public class CalcDpsProductivityByDay extends AbstractQueryService {
	...
	public ResponseObj calcDpsProductivityByDay(Map<String, Object> params) throws Exception {
		...
		for (DpsProductivity obj: dpsProductivities) {
			...
			obj.setWorkers((float)Math.round(obj.getWorkers()*100)/100);
		}
	...
	private List<DpsProductivity> getDpsWcsSum(String type, String fromDate, String toDate) throws Exception {
		...
		if (WORK_TYPE_TOTAL_PICKING.equals(type)) {
			dateSql = FnfUtils.queryCustServiceWithCheck("dps_total_picking_ps_by_day");
			queryManager = this.getDataSourceQueryManager(WmsMheHr.class);
		} else if (WORK_TYPE_LOAD_STOCK.equals(type)) {
			dateSql = FnfUtils.queryCustServiceWithCheck("dps_load_stock_ps_by_day");
		} else if (WORK_TYPE_PICKING.equals(type)) {
			dateSql = FnfUtils.queryCustServiceWithCheck("dps_picking_ps_by_day");
		} else if (WORK_TYPE_INSPECTION.equals(type)) {
			dateSql = FnfUtils.queryCustServiceWithCheck("dps_inspection_ps_by_day");
		}
		...
		for (DpsProductivity obj: dpsPrdSum) {
			...
			if (WORK_TYPE_PICKING.equals(obj.getWorkType())) {
				if ("D".equals(obj.getPackTcd())) {
					obj.setWorkType("D" + obj.getWorkType());
				} else {
					obj.setWorkType("H" + obj.getWorkType());
				}
			}
		}
	}
