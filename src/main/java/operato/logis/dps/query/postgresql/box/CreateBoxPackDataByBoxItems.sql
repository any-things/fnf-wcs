...
FROM (
	...
	, (
		SELECT 
			SUM(P.BOX_WT) / 1000 AS BOX_WT
			, SUM(P.BOX_WT_MIN) / 1000 AS BOX_WT_MIN
			, SUM(P.BOX_WT_MAX) / 1000 AS BOX_WT_MAX
		FROM (
			SELECT
				BOX_WT, BOX_WT_MIN, BOX_WT_MAX
			FROM
				BOX_TYPES
			WHERE
				DOMAIN_ID = :domainId
	...
	, (SELECT
			DISTINCT SHOP_CD, SHOP_NM, ORDER_TYPE, BOX_TYPE_CD, BOX_ID, CLASS_CD, BOX_CLASS_CD
		FROM
			JOB_INSTANCES
		WHERE
			DOMAIN_ID = :domainId
