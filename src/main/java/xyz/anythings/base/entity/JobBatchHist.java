...
@Table(name = "job_batch_hists", idStrategy = GenerationRule.UUID, indexes = {
	@Index(name = "ix_job_batch_hists_0", columnList = "domain_id,job_type,job_date,status"),
	@Index(name = "ix_job_batch_hists_1", columnList = "domain_id,wms_batch_no"),
	@Index(name = "ix_job_batch_hists_2", columnList = "domain_id,batch_group_id"),
	@Index(name = "ix_job_batch_hists_3", columnList = "domain_id,equip_type,equip_cd"),
	@Index(name = "ix_job_batch_hists_4", columnList = "domain_id,brand_cd,season_cd"),
	@Index(name = "ix_job_batch_hists_5", columnList = "domain_id,area_cd,stage_cd")
})
public class JobBatchHist extends xyz.elidom.orm.entity.basic.ElidomStampHook {
	/**
	 * SerialVersion UID
	 */
	...
	public void updateStatus(String status) {
		...
		if(ValueUtil.isEqual(JobBatchHist.STATUS_CANCEL, status)) {
	...
	public static int getMaxJobSeq(Long domainId, String comCd, String areaCd, String stageCd, String jobDate) {
		...
		List<JobBatchHist> jobSeqList = queryManager.selectList(JobBatchHist.class, condition);
	 ...
	 */
	@Transactional(propagation=Propagation.REQUIRES_NEW) 
	public static JobBatchHist createJobBatch(String batchId, String jobSeq, BatchReceipt batchReceipt, BatchReceiptItem receiptItem) {
		JobBatchHist batch = new JobBatchHist();
		batch.setId(batchId);
		...
		batch.setStatus(JobBatchHist.STATUS_RECEIVE);
